import { Machine, Grid } from './utils'
...
class Maze {
    ...
    constructor(input: string) {
        this.robot = new Machine(input)
        this.robot.run([], false)
        this.grid.set(0, 0, this.symbols[1])
    }

    ahead() {
        const ret = this.pos.slice()
    ...
    loop() {
        ...
        this.robot.continue()
        const status = this.robot.output.pop()
        // if (status == 2) status = 1
        // console.log({ status: status });
        if (status == 0) {
            // wall
            const wall = this.ahead()
            ...
            const right = this.grid.get(rightPos[0], rightPos[1])
            if (right) {
                if (right == '.' || right == 'H') {
    ...
    draw() {
        const s = this.grid.get(this.pos[0], this.pos[1])
        const robotSymbols = ['^', '>', 'V', '<']
        this.grid.set(0, 0, 'S')
        this.grid.set(this.pos[0], this.pos[1], robotSymbols[this.dir])
        const ret = this.grid.draw()
        this.grid.set(0, 0, '.')
        ...
        while (queue.length) {
            ...
            if (cell) {
                ...
                for (let i = 0; i < 4; i++) {
                    ...
                    const symbol = this.grid.get(ahead[0], ahead[1])
