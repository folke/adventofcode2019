...
export default class Day5 extends Command {
  async run() {
    try {
      ...
      var phases = "9,8,7,6,5".split(",").map(x => parseInt(x, 10));
      ...
      var permutations = permutate(phases);
      // permutations = [phases];

      var maxPhases = phases;
      var maxSignal = 0;
      ...
      for (let p = 0; p < permutations.length; p++) {
        ...
        var running = perm.length
        while(running > 0) {
          for (let i = 0; i < perm.length; i++) {
            console.log({amplifier: i, running: amplifiers[i].running})
