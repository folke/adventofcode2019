...
function param(program: number[], programIdx: number, paramIdx: number) {
  const modes = program[programIdx].toString()
    .slice(0,-2).split('').map(x=>parseInt(x, 10)).reverse()
...
function intCode(program: number[], input: number = 0) {
  var halt = false
  var skip = 4;
