...
export default class Day11 extends Command {
  async run() {
    ...
    var canvas: number[][] = new Array(height)
    for(let i = 0; i < height; i++) {
      canvas[i] = new Array(width)
      canvas[i].fill(0)
    }

    var counts: number[][] = new Array(height)
    for(let i = 0; i < height; i++) {
      counts[i] = new Array(width)
      counts[i].fill(0)
    }

    var robot = new Computer(intCode)
    ...
    for(let row = topLeft[1]; row < height; row++) {
      var line = canvas[row].slice(topLeft[0]).map(x => x == 0 ? ' ' : '*').join('').trimRight()
