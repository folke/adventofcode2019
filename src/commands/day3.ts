...
function traceWire(wire:string) {
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    var count = parseInt(step.slice(1))
...
export default class Day3 extends Command {
  ...
  async run() {
    ...
    intersection.forEach(p => {
      // part 1
      // // console.log(p)
      // var coord = p.split(' ').map(x => parseInt(x, 10))
      // var d = Math.abs(coord[0]) + Math.abs(coord[1])
      // // console.log([coord, d])

      // part 2

      var d = w1.findIndex(x => x == p) + w2.findIndex(x => x == p) + 2
