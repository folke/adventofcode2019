...
class Space {
  ...
  constructor(input: string) {
    this.space = input.split("\n").map(x => x.trim()).filter(x => x != '').map(row => row.split("").map(x => x == '#' ? 1 : 0))
  ...
  tracePosition(position: number[]) {
    ...
    for(let vx = 1; vx < this.width; vx++) {
      for(let vy = 1; vy < this.height; vy++) {
        var d = gcd(Math.abs(vx), Math.abs(vy))
        if (d > 1) continue
    ...
    vectors.sort((a, b) => {
      let da = rot(Math.atan2(a[1], a[0]) * (180/Math.PI), -90)
      let db = rot(Math.atan2(b[1], b[0]) * (180/Math.PI), -90)
      return da - db
    })

    var done = 0;
    vectors.forEach(v => {
      let asteroid = this.findAsteroid(position, v)
      if (asteroid) {
...
The 299th and final asteroid to be vaporized is at 11,1.
    ...
    var test = [1, 2,3,10,20,50,100,199,200,201,299]
    test.forEach(d => {
