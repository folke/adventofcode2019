...
export default class Day12 extends Command {
  async run() {
    class Moon {
      pos: number[];
      vel: number[];
    ...
    for(let step = 0; step <= steps; step++) {
      ...
      if (step > 0) {
        ...
        if (allFound) {
          var d = gcd(cycle[0], gcd(cycle[1], cycle[2]))
          console.log({
            cycle: cycle,
            gcd: d,
      ...
      for(let m = 0; m < moons.length; m++) {
        for(let om = 0; om < moons.length; om++) {
          if (om == m) continue
          for(let i = 0; i < 3; i++) {
            if (moons[m].pos[i] == moons[om].pos[i])
              continue
            if (moons[m].pos[i] > moons[om].pos[i])
              moons[m].vel[i]--
            else
    ...
    var totalEnergy = moons.map(m => m.energy()).reduce((p, c) => p + c)
