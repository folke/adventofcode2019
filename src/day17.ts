...
const output = camera.output.map(x => String.fromCharCode(x))
...
console.log({ bounds: grid.bounds, width: grid.width, height: grid.height })
...
console.log(grid.draw())
console.log({ part1: alignmentSum })
...
class Robot {
    x = 0
    y = 0
    d = 0 // Starting Up, clockwise
    constructor(public grid: Grid<string>) {
        grid.walk((x, y, v) => {
            if ('^>v<'.includes(v)) {
                this.x = x
                this.y = y
    ...
    private _ahead(x = this.x, y = this.y, d = this.d): [number, number] {
        const dd = [
            [0, -1],
            [1, 0],
            [0, 1],
        ...
        return [x + dd[0], y + dd[1]]
    }

    peek() {
        const ahead = this._ahead()
        return this.grid.get(...ahead)
    }
    ...
    move() {
        ...
        this.x = ahead[0]
        this.y = ahead[1]
        this.grid.set(this.x, this.y, '^>v<'.charAt(this.d))
    }
...
function ngrams(movesLine: string) {
    ...
    const counts2: [string, number, number][] = [...counts.entries()].map(v => {
        return [v[0], v[1], v[0].length * v[1]]
    })
    return counts2.sort((a, b) => b[2] - a[2])
}
...
function search(
    ...
    for (let f = 0; f < Math.min(maxDepth, counts.length); f++) {
        const F = counts[f]
        const childLine = line.split(F[0]).join(`${s},${s}`)
        const childFunctions = functions.slice()
        childFunctions.push(F[0])
        if (depth == 2) {
            if (childLine.match(/^[ABC,]*$/g))
                return [
                    childLine
                        .replace(/A,A/g, 'A')
                        .replace(/B,B/g, 'B')
        ...
        if (depth < 2) {
            const ret = search(childLine, childFunctions, depth + 1)
            if (ret) return ret
        }
    }
}

// moves = 'R,8,R,8,R,4,R,4,R,8,L,6,L,2,R,4,R,4,R,8,R,8,R,8,L,6,L,2'.split(',')
const functions = search(moves.join(','), [])
if (functions) {
    console.log(functions)
    const robot = new Machine(code)
    robot.run([], false)
    robot.program[0] = 2
    robot.continue()
    console.log(robot.output.map(x => String.fromCharCode(x)).join(''))
    robot.output = []
    for (const f of functions) {
        const encoded = f.split('').map(x => +x.charCodeAt(0))
        encoded.push(10)
        console.log({ f: f, input: encoded })
        robot.input = encoded
        robot.continue()
        console.log(robot.output.map(x => String.fromCharCode(x)).join(''))
        robot.output = []
    }
    robot.input = ['y'.charCodeAt(0), 10] // without live feed, there's no collected dust at the end
    robot.continue()
    while (robot.running) {
        robot.continue()
    }
    console.log(robot.output.map(x => String.fromCharCode(x)).join(''))
    console.log(robot.output.pop())
}
